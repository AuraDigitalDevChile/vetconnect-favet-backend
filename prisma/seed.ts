...
async function main() {
  ...
  // Verificar si ya existen sesiones de telemedicina antes de crear
  const sesionesExistentes = await prisma.sesionTelemedicina.count();

  if (sesionesExistentes > 0) {
    console.log('⚠️  Ya existen sesiones de telemedicina. Saltando creación...\n');
    return;
  }

  // Crear citas de tipo TELEMEDICINA solo si no existen sesiones
  const citaTelemed1 = await prisma.cita.create({
    data: {
      centro_id: centroBilbao.id,
      paciente_id: paciente5.id, // Bella (Labrador)
      tutor_id: tutor3.id,
      veterinario_id: vet1.id,
      tipo: 'TELEMEDICINA',
  ...
  const citaTelemed2 = await prisma.cita.create({
    data: {
      centro_id: centroBilbao.id,
      paciente_id: paciente4.id, // Michi (gato mestizo)
      tutor_id: tutor2.id,
      veterinario_id: vet2.id,
      tipo: 'TELEMEDICINA',
  ...
  const citaTelemed3 = await prisma.cita.create({
    data: {
      centro_id: centroElRoble.id,
      paciente_id: paciente7.id, // Simba (Maine Coon)
      tutor_id: tutor5.id,
      veterinario_id: vet3.id,
      tipo: 'TELEMEDICINA',
      ...
      observaciones: 'Consulta sobre plan alimenticio para gato de raza grande',
    },
  });
  ...
  // Crear sesión 2 - EN_CURSO (simulando que ya inició)
  ...
  const sesion3 = await prisma.sesionTelemedicina.create({
    data: {
      ...
      notas_sesion: 'Sesión finalizada exitosamente. El tutor consultó sobre plan alimenticio para Maine Coon. Se recomendó alimento premium para razas grandes con control de peso.',
      diagnostico: 'Gato en buen estado general. Peso ligeramente por encima del ideal (7.8kg). Se recomienda dieta controlada.',
      recetas_generadas: 1,
      examenes_solicitados: 0,
    },
  });
  ...
  console.log('✅ 3 sesiones de telemedicina creadas (1 programada, 1 en curso, 1 finalizada)\n');
  ...
  console.log(`   • 6 Citas programadas (3 regulares + 3 telemedicina)`);
  console.log(`   • 3 Sesiones de telemedicina (1 programada, 1 en curso, 1 finalizada)`);
